function cell_data = protein_read_folder(dir_name, CEN_LINE_OFFSET, POLE_PORTION)
...
fprintf('Group into %d color combined images {DAPI, FITC, TexRd}.\n\n', length(TIFF_list)/3);


fprintf(2,'<strong>Instructions</strong>:\n');
fprintf(['1. Rotate image to where the cell is postioned vertically (Leading->Trailing),\n'...
    '2. Draw box to encompass the cell \n\t(2 horizontal, 2 vertical, 1 vertical center line).\n']);
...
cell_data = cell(1, length(TIFF_list)/3);
% Check if the naming of the files are consistent for images taken from our
% new scope. 
for i = 1:length(TIFF_list);
    file_id = TIFF_list{i}(1:(length(dir_name) + 5));
    if ~strcmp(file_id(1:length(dir_name)), dir_name);  % compare strings
...
for i = 1:3:length(TIFF_list);
    file_id = TIFF_list{i}(1:(length(dir_name) + 5));
    fprintf('[<strong>%d</strong> of %d] Processing: %s *3.TIF, ...', ...
        (i-1)/3+1, length(TIFF_list)/3, file_id);
    
    cell_data{(i-1)/3+1} = ...
        cell_box_select([dir_name, '/', file_id], CEN_LINE_OFFSET, POLE_PORTION);
    close all;
    
    if cell_data{(i-1)/3+1}.is_bad;
        fprintf(' Considered ');
        fprintf(2, 'BAD ');
        fprintf(', ignored!\n');
    else
