function [ratio_poles, std_poles, ratio_body, std_body] = pr_loc_compare_ratio(dir_name, POLE_PORTION, y_lim)
...
for i = 1:length(dir_name);
    ...
    for j = 1:length(obj_all);
        ...
        pole_ratios(idx).ratios = pr_loc_divide_pole(spd_data_cell{i}.spd_data, spd_data_cell{i}.(obj_all{j}).list_chosen, POLE_PORTION);
