function [protein_data, ratio_box_mean, ratio_box_std, ratio_line_mean, ratio_line_std] = ...
    pr_loc_average_ratio(protein_data, list_good, NUM_BIN)
...
for i = 1:length(list_good);
    ...
    if isfield(protein_data{idx}, 'ratio_box_100') && isfield(protein_data{idx}, 'ratio_line_100');
        ratio_box_all(i, :) = protein_data{idx}.ratio_box_100;
        ratio_line_all(i, :) = protein_data{idx}.ratio_line_100;
    else
        ratio_box_sub = protein_data{idx}.ratio_box;
        ratio_line_sub = protein_data{idx}.ratio_line;
        x_sub = protein_data{idx}.plot_x;
        ...
        protein_data{idx}.ratio_box_100 = ratio_box_all(i, :);
        protein_data{idx}.ratio_line_100 = ratio_line_all(i, :);
    end;
end;
